...
```shell
pkg update && yes | pkg upgrade && yes | pkg install git && cd $HOME && git clone "https://github.com/TheLastAirbendr/mysql-for-termux" && cd mysql-for-termux && chmod u+x installer.sh && ./installer.sh && source $HOME/../usr/etc/bash.bashrc
```
